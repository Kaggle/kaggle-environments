...
void OpenGLRenderer3D::SwapBuffers() {
  ...
  glReadPixels(0, 0, context_width, context_height, GL_RGB, GL_UNSIGNED_BYTE,
                       &last_screen_[0]);
}
...
void OpenGLRenderer3D::RenderOverlay2D(
  ...
  glBindVertexArray(overlayBuffer.vertexArrayID);
  SetTextureUnit(0);
...
void OpenGLRenderer3D::RenderOverlay2D() {
 ...
 // assert(context);
  ...
  glBindVertexArray(quadBuffer.vertexArrayID);
  glDrawArrays(GL_TRIANGLES, 0, 6);
  glBindVertexArray(0);
...
static const int MAX_DEVICES = 4;
  ...
  int major, minor;
  EGLBoolean success;
  RETURN_IF_EGL_ERROR(success = eglInitialize(egl_display, &major, &minor));
  std::cout << "QUAKE: " << major << " " << minor << std::endl;

//  RenderTriangle();
//  exit(1);
//  EGLDisplay egl_display = eglGetDisplay(EGL_DEFAULT_DISPLAY);
  if (egl_display == EGL_NO_DISPLAY) {
    std::cout << "Failed to find valid EGL display." << std::endl;
    return false;
  }

  RETURN_IF_EGL_ERROR(success = eglBindAPI(EGL_OPENGL_API));
  EGLint num_configs;
...
// RETURN_IF_EGL_ERROR(egl_surface = eglCreateWindowSurface(egl_display, egl_config, EGL_DEFAULT_DISPLAY, 0));
  ...
  RETURN_IF_EGL_ERROR(egl_context = eglCreateContext(egl_display, egl_config,
...
/*
  ...
  std::cout << "EGLDisplay: " << eglDpy << std::endl;
  EGLint major, minor;
  RETURN_IF_EGL_ERROR(eglInitialize(eglDpy, &major, &minor));
  std::cout << "eglInitialize: " << major << " " << minor << std::endl;
  std::cout << "QUAKE 1" << std::endl;
  EGLint numConfigs;
  EGLConfig eglCfg;
  RETURN_IF_EGL_ERROR(eglChooseConfig(eglDpy, configAttribs, &eglCfg, 1, &numConfigs));
  std::cout << "QUAKE 2" << std::endl;

  EGLSurface eglSurf = eglCreatePbufferSurface(eglDpy, eglCfg, pbufferAttribs);
  RETURN_IF_EGL_ERROR(eglBindAPI(EGL_OPENGL_API));

  EGLContext eglCtx = eglCreateContext(eglDpy, eglCfg, EGL_NO_CONTEXT,
                                       NULL);
  eglMakeCurrent(eglDpy, eglSurf, eglSurf, eglCtx);
  std::cout << "QUAKE 3" << std::endl;*/
   ...
   // }
    ...
    if (!higherThan32 && exit_on_version_too_low) {
      std::cout << "VERSION TOo LOW: " << glVersion[0] << " " << glVersion[1] << std::endl;
//      return false;
    }
...
void OpenGLRenderer3D::UpdateTexture(int textureID, SDL_Surface *source,
  ...
  SDL_LockSurface(source);
  glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, w, h, type2, GL_UNSIGNED_BYTE,
          source->pixels);
  SDL_UnlockSurface(source);
...
void OpenGLRenderer3D::LoadShader(const std::string &name,
  ...
  if (info != GL_TRUE || glIsShader(shader.vertexShaderID) != GL_TRUE) {
    DO_VALIDATION;
    printf("%d %d\n", info, glIsShader(shader.vertexShaderID));
  ...
  if (info != GL_TRUE ||
      glIsShader(shader.fragmentShaderID) != GL_TRUE) {
    DO_VALIDATION;
    Log(e_FatalError, "OpenGLRenderer3D", "LoadShader",
  ...
  if (name == "overlay") {
    DO_VALIDATION;
    glBindAttribLocation(shader.programID, 0, "vertex");
    glBindFragDataLocation(shader.programID, 0, "stdout");
  }
...
void OpenGLRenderer3D::SetUniformInt(const std::string &shaderName,
  ...
  if (iter != uniformCache.end()) {
    ...
    location = glGetUniformLocation((*shaderIter).second.programID,
                                            varName.c_str());
    if (location == -1)
...
void OpenGLRenderer3D::SetUniformMatrix4(const std::string &shaderName,
  ...
  glUniformMatrix4fv(location, 1, true,
