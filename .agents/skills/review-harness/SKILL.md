...
| **Overloaded glyph in board rendering** | The ASCII board uses the same character for two structurally distinct pieces of state, disambiguated only by *which row of the rendering you're on*. Models routinely lose the row-class context and misclassify the glyph, then propose moves they think are legal but aren't. Example: dots and boxes rendered open horizontal edges as `.` in dot-rows AND unclaimed box cells as `.` in vertical-edge-rows — 8/8 forfeits in a sampled archive were the model picking an already-drawn edge, often one in its own move history; deepseek traces showed it literally reasoning "`h(2,2)` is `.`, which denotes an open horizontal edge" while the JSON state had that edge drawn. The legend documents the overload but doesn't fix the perceptual problem. | Render the prompt at a non-trivial mid-game state. Tabulate every glyph in the board block and what it can mean depending on which row/column class it sits in. Any glyph with two semantically distinct meanings is a finding — even if the legend explains it, the model still has to do row-class arithmetic on every cell to decode. Cross-check by reading replay `thoughts` for traces that confidently misclassify cells. | Assign each semantic role its own unique glyph (e.g. `.` for open edges, `*` for open boxes; or `:` vs `.` to distinguish open vertical vs horizontal edges). Update the legend AND verify the rendered output by eye that no two classes share a character. Bonus: add axis labels so the model doesn't have to count columns to locate a glyph's coordinates. |
