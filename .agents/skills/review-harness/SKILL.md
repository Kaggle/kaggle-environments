...
| **Rethink suffix uses the wrong shape for the failure** | Parse failures come in two flavours and need two different rethinks. (1) **Empty `previous_response`** (parser found nothing extractable): the model needs the output-format spec restated — it's the format that failed. (2) **Non-empty `previous_response`** (model produced parseable JSON but the move was illegal): the model already complied with the format; restating it is noise. The model needs to see its own previous answer plus the illegal move and pick something legal. Many harnesses use a single suffix for both cases: either no format reminder at all (breaks unparseable retries) or a format reminder on every retry (wastes tokens on illegal-move retries and dilutes the actual correction signal). | Render both cases and read them: `make_prompt(obs, [], previous_response=None, previous_action=None)` vs `make_prompt(obs, [], previous_response="...", previous_action="...")`. The unparseable case must restate the output format; the illegal-move case must show the previous response and attempted move and ask for a legal one (and should NOT restate the format). | Branch `RETHINK_SUFFIX` selection on `bool(previous_response)`. Use two named templates (e.g. `RETHINK_UNPARSABLE`, `RETHINK_ILLEGAL`) — the create-harness skill has the canonical shape. |
