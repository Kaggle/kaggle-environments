...
// 3. Main logic to determine which command to run
if (gameArg) {
  // If the command is 'build --all', use pnpm's recursive build with filtering.
  // This is the FIX: pnpm will figure out the dependency graph and build
  // shared packages (like '@kaggle-environments/core') before the visualizers.
  if (command === "build" && gameArg === "--all") {
    ...
    try {
      // Use execSync to run the single, powerful pnpm command.
      // 'stdio: inherit' ensures the build output is shown in the console.
      execSync(buildCommand, { stdio: "inherit" });
      console.log("\n✅ All visualizers built successfully.");
    } catch (error) {
      console.error("\n❌ Build failed.");
      // Exit with a non-zero code to fail the CI step.
      process.exit(1);
    }
  } else if (gameArg === "--all") {
    // For other commands like 'dev --all', run them in parallel (original behavior).
    console.log(`Running "${command}" for all ${gameVisualizers.length} visualizers...`);
    gameVisualizers.forEach(runCommand);
  } else {
    // Run command for a single, specified game.
  ...
  // 4. If no game is specified, show the interactive prompt.
  (async () => {
