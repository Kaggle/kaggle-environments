...
export function renderer(context: any) {
  console.log(context);
  ...
  const players = [info?.TeamNames?.[0] || 'Player 0', info?.TeamNames?.[1] || 'Player 1'];
      ...
      for (let k = 0; k < 2; k++) {
        ...
        button.addEventListener('click', () => {
          for (let i = 0; i < visList.length; i++) {
            for (let j = 0; j < 2; j++) {
              visList[i].current.players[j].ramainingTime =
                context.environment.steps[i][j].observation.remainingOverageTime;
            }
          }
