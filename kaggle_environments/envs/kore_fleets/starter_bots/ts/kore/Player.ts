...
export class Player {
  ...
  public get shipyards(): Shipyard[] {
    return Array.from(this.board.shipyards.values()).filter((shipyard) =>
      this.shipyardIds.some((sId) => sId == shipyard.id)
    );
  }
  ...
  public get fleets(): Fleet[] {
    return Array.from(this.board.fleets.values()).filter((fleet) => this.fleetIds.some((fId) => fId == fleet.id));
  }
  ...
  public get nextActions(): Map<String, ShipyardAction> {
    ...
    this.shipyards
      .filter((shipyard) => shipyard.nextAction)
      .forEach((shipyard) => result.set(shipyard.id, shipyard.nextAction as ShipyardAction));
    return result;
  }
