...
export class Board {
  ...
  public static fromRaw(rawObservation: string, rawConfiguration: string): Board {
    ...
    for (let playerId = 0; playerId < observation.playerHlt.length; playerId++) {
      ...
      playerFleets.forEach((fleetStrs, fleetId) => {
        ...
        board.addFleet(
          new Fleet(fleetId, shipCount, fleetDirection, fleetPosition, fleetKore, flightPlan, playerId, board)
        );
      });
      ...
      playerShipyards.forEach((shipyardInts, shipyardId) => {
        ...
        board.addShipyard(
          new Shipyard(shipyardId, shipCount, shipyardPosition, playerId, turnsControlled, board, undefined)
        );
      });
    }
    return board;
  ...
  public resolveCollision(fleets: Fleet[]): Pair<Fleet | undefined, Fleet[]> {
    ...
    if (largestFleets.length == 1) {
      ...
      return new Pair<Fleet | undefined, Fleet[]>(
        winner,
        fleets.filter((f) => !(f.id == winner.id))
      );
    }
  ...
  public next(): Board {
    ...
    for (let player of board.players) {
      ...
      for (let shipyard of player.shipyards) {
        ...
        if (
          nextAction.actionType == ShipyardAction.SPAWN &&
          player.kore >= spawnCost * nextAction.shipCount &&
          nextAction.shipCount <= shipyard.maxSpawn
        ) {
          player.kore -= spawnCost * nextAction.shipCount;
      ...
      for (let fleet of player.fleets) {
        ...
        if (
          fleet.flightPlan.length > 0 &&
          fleet.flightPlan.startsWith('C') &&
          fleet.shipCount >= converstCost &&
          fleet.cell.shipyardId.length == 0
        ) {
    ...
    fleetCollisionGroups.forEach((collidedFleets, positionIdx) => {
      ...
      if (winnerOptional) {
        ...
        const maxEnemySize =
          deleted.length > 0 ? deleted.map((f) => f.shipCount).reduce((a, b) => (a > b ? a : b), 0) : 0;
    ...
    for (let shipyard of Array.from(board.shipyards.values())) {
      ...
      if (optFleet && (optFleet as Fleet).playerId != shipyard.playerId) {
        ...
        if (fleet.shipCount > shipyard.shipCount) {
          ...
          board.addShipyard(
            new Shipyard(this.createUid(), count, shipyard.position, fleet.player.id, 1, board, undefined)
          );
    ...
    for (const fleet of Array.from(board.fleets.values())) {
      ...
      const deltaKore = Board.roundToThreePlaces(cell.kore * Math.min(fleet.collectionRate, 0.99));
      if (deltaKore > 0) {
