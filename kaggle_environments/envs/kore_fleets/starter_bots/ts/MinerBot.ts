  ...
  for (let shipyard of me.shipyards) {
    if (remainingKore > 1000 && shipyard.maxSpawn > 5) {
      if (shipyard.shipCount >= convertCost + 10) {
        ...
        shipyard.setNextAction(
          ShipyardAction.launchFleetWithFlightPlan(
            Math.min(convertCost + 10, Math.floor(shipyard.shipCount / 2)),
            flightPlan
          )
        );
        ...
        shipyard.setNextAction(
          ShipyardAction.spawnShips(Math.min(shipyard.maxSpawn, Math.floor(remainingKore / spawnCost)))
        );
      }
