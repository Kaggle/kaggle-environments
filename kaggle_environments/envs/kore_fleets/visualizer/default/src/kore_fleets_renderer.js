...
export function renderer({
  ...
  const colors = {
    ...
    ships: [
      ['#35A0FF', '#1A85FF', '#026FF2', '#0064EA'],
      ['#C02858', '#A50D45', '#8D0038', '#7D0030'],
      ['#FFFF73', '#FEFE60', '#E6E853', '#D6DD4B'],
      ['#FE8ADA', '#E36FC7', '#CB59BA', '#BB4EB2'],
    ],
  };
  ...
  const drawRects = (ctx, rects, color, scale = 1, gridSize = 20, drawFrame = -1) => {
  ...
  // Expand storage.
  const { cellInset, cellScale, cellSize, fixedCellSize, koreRotations, maxCellKore, xOffset, yOffset } = data(
    bufferCanvas,
    'storage'
  );
  ...
  if (step > 1) {
    ...
    players.forEach((player, playerIndex) => {
      ...
      Object.entries(shipyards).forEach(
        ([uid, [pos, shipCount, turnsControlled]]) => (board[pos].shipyard = playerIndex)
      );
  ...
  const drawFleetLaunches = (ctx, player, playerIndex, x, y, reverse) => {
    const actions = replay.steps[step][playerIndex].action || {};
    const launches = Object.values(actions)
      .filter((a) => a.includes('LAUNCH'))
      .map((a) => a.substring(7).replace(/_/, ' '))
