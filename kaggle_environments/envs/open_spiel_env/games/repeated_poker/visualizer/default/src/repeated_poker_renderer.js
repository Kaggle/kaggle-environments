export function renderer(options) {
  ...
  `;
    ...
    function _parseStepHistoryData(universalPokerJSON) {
        ...
        // Parse win odds
        const p0WinOdds = Number(universalPokerJSON.odds[0]).toLocaleString(undefined, {
            style: 'percent',
            minimumFractionDigits: 2
        });
        const p1WinOdds = Number(universalPokerJSON.odds[1]).toLocaleString(undefined, {
            style: 'percent',
            minimumFractionDigits: 2
        });
    ...
    function _parseKagglePokerState(options) {
        ...
        // This is the source of truth for the current state, derived from the filtered agent steps.
        const currentUniversalPokerJSON = _getCurrentStepUniversalPokerJSON(options);
        const currentUniversalPokerState = currentUniversalPokerJSON;
