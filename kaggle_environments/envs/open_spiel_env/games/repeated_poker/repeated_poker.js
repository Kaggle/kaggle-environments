function renderer(options) {
  ...
  function _parseStepHistoryData(universalPokerJSON) {
    ...
    // Parse win odds
    p0WinOdds = Number(universalPokerJSON.odds[0]).toLocaleString(undefined, {
      style: 'percent',
      minimumFractionDigits: 2,
    });
    p1WinOdds = Number(universalPokerJSON.odds[1]).toLocaleString(undefined, {
      style: 'percent',
      minimumFractionDigits: 2,
    });
  ...
  function _getCurrentStepUniversalPokerJSON(options) {
    ...
    const agentSteps = environment.info.stateHistory.filter(
      (s) => JSON.parse(JSON.parse(s).current_universal_poker_json).current_player !== -1
    );
  ...
  function _parseKagglePokerState(options) {
    ...
    const board_cards = currentStepFromStateHistory.communityCards
      ? currentStepFromStateHistory.communityCards.match(/.{1,2}/g).reverse()
      : [];
