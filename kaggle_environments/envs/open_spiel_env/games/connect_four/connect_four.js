function renderer(options) {
  ...
  function _renderBoardDisplay_svg(gameStateToDisplay, displayRows, displayCols) {
    ...
    if (
      ...
      gameStateToDisplay.board.length === 0
    ) {
    ...
    for (let r_data = 0; r_data < displayRows; r_data++) {
      ...
      for (let c_data = 0; c_data < displayCols; c_data++) {
        // c_data iterates through columns of `board[r_data]`
        ...
        const circleElement = currentBoardSvgElement.querySelector(
          `#cell-${visual_svg_row_index}-${visual_svg_col_index}`
        );
