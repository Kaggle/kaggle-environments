...
interface RendererProps {
  replay: ReplayData<ConnectFourStep[]>;
  step: number;
  agents: any[];
}
...
// --- Helper to get board from transformed step ---
const getBoardFromStep = (currentStep: ConnectFourStep, config: Record<string, any>): number[] => {
  const columns = config.columns || 7;
  const rows = config.rows || 6;

  const board2D = currentStep?.boardState?.board;
  if (!board2D || !Array.isArray(board2D) || board2D.length === 0) {
    return new Array(rows * columns).fill(0);
  }

  // Convert 2D board (rows x columns) with ".", "x", "o" to flat array with 0, 1, 2
  // Note: In the state, board[0] is the bottom row (where pieces first fall)
  // We reverse the row order so that the bottom row in data becomes the bottom row on screen
...
// --- Helper to get player names from replay ---
const getPlayerNames = (replay: ReplayData<ConnectFourStep[]>): [string, string] => {
  for (const step of replay.steps || []) {
    if (step?.players?.length >= 2) {
      return [step.players[0]?.name || "Player 1", step.players[1]?.name || "Player 2"];
    }
  }
...
const GameStatus: FunctionComponent<RendererProps> = ({ replay, step }) => {
  ...
  // Find winner from players
  const winnerPlayer = currentStep.players.find((p) => p.reward === 1);
  if (winnerPlayer) {
    const winnerIndex = currentStep.players.indexOf(winnerPlayer);
    tokenUrl = getTokenAsDataURL(winnerIndex + 1);
    message = `Winner: ${winnerPlayer.name}`;
  } else if (currentStep.players.every((p) => p.reward === 0)) {
    message = "Draw";
  }
...
export const Renderer: FunctionComponent<RendererProps> = ({ replay, step, agents }) => {
  ...
  useEffect(() => {
    ...
    const currentStep = replay.steps[step];
    const prevStepData = step > 0 ? replay.steps[step - 1] : currentStep;

    const board = getBoardFromStep(currentStep, config);
    ...
    const draw = (frame: number) => {
      ...
      for (let i = 0; i < board.length; i++) {
        ...
        let yFrame = 1;
        if (!isBackStep && step > 0 && prevBoard[i] !== board[i]) {
      ...
      // Check for terminal state using the transformed step format
      if (currentStep?.isTerminal) {
        for (let i = 0; i < board.length; i++) {
          for (let d = 0; d < 8; d++) {
  ...
  return html`
    <div class="renderer-container">
      <div class="player-legend" style="display: flex; align-items: center; justify-content: center; gap: 32px; padding: 12px; color: white; font-family: sans-serif; font-size: 15px; flex-shrink: 0;">
