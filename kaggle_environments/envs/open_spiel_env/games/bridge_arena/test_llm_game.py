"""Run a full Bridge Arena game with LLM agents for local integration testing.

Usage:
    GEMINI_API_KEY=... uv run python -m \\
        kaggle_environments.envs.open_spiel_env.games.bridge_arena.test_llm_game
"""
...
def run_llm_game():
    ...
    # AABB seating: team A = same agent on seats 0+1 (N/S);
    # team B = same agent on seats 2+3 (E/W). Run two copies of the
    # same harness for both teams (self-play); swap one to compare models.
    print("Running Bridge Arena game with LLM agents (AAvBB self-play)...")
