"""Run a full Quoridor game with LLM agents for local integration testing.

Usage:
    GEMINI_API_KEY=... uv run python -m \\
        kaggle_environments.envs.open_spiel_env.games.quoridor.test_llm_game
"""
...
def run_llm_game():
    ...
    if "MODEL_PROXY_URL" not in os.environ:
        ...
        print("MODEL_PROXY_URL not set -- using dummy_url; the harness will call the provider directly.")
    ...
    env = make(
        ...
        configuration={
            ...
            "openSpielGameParameters": {"board_size": 5, "wall_count": 3},
        },
        debug=True,
    )
    ...
    print("Running Quoridor game with LLM agents...")
    ...
    # Save to a separate filename so we don't clobber the checked-in
    # ``test-replay.json`` that the e2e tests load.
