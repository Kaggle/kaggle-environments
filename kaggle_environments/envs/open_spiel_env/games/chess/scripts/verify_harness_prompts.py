"""Verify that the migrated chess harness produces the same prompts as the old one.

Walks an old-format Kaggle chess replay JSON, recreates the game move-by-move,
...
Usage:
    python3 -m kaggle_environments.envs.open_spiel_env.games.chess.scripts.verify_harness_prompts \\
        /path/to/old_replay.json

    # Or fetch a sample replay first:
    curl -o /tmp/old.json https://www.kaggleusercontent.com/episodes/74677823.json
    python3 -m kaggle_environments.envs.open_spiel_env.games.chess.scripts.verify_harness_prompts \\
        /tmp/old.json
"""
