...
test.describe('Ant Foraging Visualizer', () => {
  ...
  test('renders the game', async ({ page }) => {
    ...
    const players = page.locator('.header .ant-player');
  ...
  test('displays terminal status at final step', async ({ page }) => {
    ...
    await expect(page.locator('.status-container').filter({ hasText: /Game over/ })).toBeVisible();
  });
});
