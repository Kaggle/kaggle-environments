...
function renderer({
  ...
  if (!parent.querySelector('#buffer')) {
    if (interactive) {
      ...
      document.addEventListener('keydown', (evt) => {
        if (!isInteractive() || evt.ctrlKey || evt.shiftKey || evt.altKey || evt.metaKey) return;
  ...
  for (let i = 0; i < rows * columns; i++) {
    ...
    const drawPiece = (sRow, sCol, angle, flip) => drawImage(bufferCanvas, c, sRow, sCol, row, col, angle, flip);
  ...
  if (agents.length && (!agents[0].color || !agents[0].image)) {
    agents.forEach((agent) => {
      ...
      ctx.drawImage(bufferCanvas, cellSize * agent.index, cellSize * 3, cellSize, cellSize, 0, 0, 100, 100);
