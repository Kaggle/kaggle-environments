...
export function renderer(context: LegacyRendererOptions) {
  ...
  const wrappedText1 = wrapText(ctx, team1_question, team1_x, question_y, row_width, line_height);
  wrappedText1.forEach(function (item) {
    ctx.fillText(item[0], item[1], item[2]);
  });
  const wrappedText2 = wrapText(ctx, team2_question, team2_x, question_y, row_width, line_height);
