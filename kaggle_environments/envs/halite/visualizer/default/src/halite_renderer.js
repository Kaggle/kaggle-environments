...
export function renderer({
  ...
  const colors = {
    ...
    ships: [
      ['#F1E61D', '#E2CD13', '#C0AE10', '#AA990E', '#716609', '#605708', '#161616'],
      ['#F97575', '#F24E4E', '#CE4242', '#B63B3B', '#792727', '#672121', '#161616'],
      ['#4BCF27', '#34BB1C', '#299516', '#268814', '#17560C', '#14470A', '#161616'],
      ['#BA4DF2', '#7B33E2', '#692BC0', '#5C26AB', '#3D1971', '#341561', '#161616'],
    ],
  };
  ...
  const getShipDir = (playerIndex, uid) => {
    for (let s = step; s >= 0; s--) {
      const action = replay.steps[s][playerIndex].action || {};
      if (uid in action) return Math.max(directions.indexOf(action[uid]), 0);
    }
    for (let s = step + 1; s < replay.steps.length; s++) {
      const action = replay.steps[s][playerIndex].action || {};
      if (uid in action) return Math.max(directions.indexOf(action[uid]), 0);
    }
    return 0;
  ...
  // Expand storage.
  const { cellInset, cellScale, cellSize, fixedCellSize, haliteRotations, maxCellHalite, xOffset, yOffset } = data(
    bufferCanvas,
    'storage'
  );
  ...
  const renderHalite = (ctx, pos, halite, maxHalite, scaleFactor, rotate) => {
    ...
    move(ctx, { x: dx, y: dy, width: ds, height: ds, angle: rotate ? haliteRotations[pos] : 0 }, () =>
  ...
  if (step > 0) {
    ...
    players.forEach((player, playerIndex) => {
      ...
      Object.values(shipyards).forEach((pos) => (board[pos].shipyard = playerIndex));
      Object.entries(ships).forEach(([uid, [pos]]) => (board[pos].ship = uid));
    });
    replay.steps[step - 1][0].observation.players.forEach((player, playerIndex) => {
      const status = state[playerIndex].status;
      const [, shipyards, ships] = player;
      const action = replay.steps[step][playerIndex].action || {};
      ...
      Object.entries(action).forEach(([uid, value]) => {
        if (value === 'SPAWN') {
          if (!board[shipyards[uid]].ship || parseInt(board[shipyards[uid]].ship.split('-')[0]) !== step) {
  ...
  const drawShip = (ctx, playerIndex, x, y, iconSize = scoreboardShipSizePx) =>
    ctx.drawImage(bufferCanvas, 500 + 100 * playerIndex, 0, fixedCellSize, fixedCellSize, x, y, iconSize, iconSize);
  const drawShipYard = (ctx, playerIndex, x, y, iconSize = scoreboardShipSizePx) =>
