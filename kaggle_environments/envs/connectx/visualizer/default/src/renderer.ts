...
export function renderer({ parent, step, agents, replay }: ConnectxOptions) {
  ...
  const draw = (frame: number) => {
    ...
    for (let i = 0; i < board.length; i++) {
      ...
      let yFrame = 1;
      if (!isBackStep && step > 0 && steps[step - 1][0].observation.board[i] !== board[i]) {
    ...
    if (steps[step].some((s) => s.status === 'DONE')) {
      for (let i = 0; i < board.length; i++) {
        for (let d = 0; d < 8; d++) {
  ...
  if (!isLastStep) {
    ...
    if (winnerStep) {
      ...
      message = winnerName ? `Winner: ${winnerName}` : `Winner: Player ${winnerIndex + 1}`;
