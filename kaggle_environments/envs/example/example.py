...
def renderer(last_step, env):
    board = ""

    for i, step in enumerate(env.steps):
        actions = [agent.action for agent in step]
        rewards = [agent.reward for agent in step]
...
json_path = path.abspath(path.join(dir_path, "example.json"))
with open(json_path) as json_file:
    specification = json.load(json_file)


def html_renderer():
    js_path = path.abspath(path.join(dir_path, "example.js"))
