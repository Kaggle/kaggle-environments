...
- Shed / inventory: `"PICKUP" <item> [n]` (from shed), `"PLACE" <item> [n]` (places an animal on a matching structure when standing on it, or drops items into the shed when adjacent to it), `"DROP"` (when shed-adjacent, dumps entire inventory into shed; overflow past `shedCapacity` discarded). "Shed-adjacent" means standing on one of the four center tiles — `(4,4)`, `(5,4)`, `(4,5)`, `(5,5)` at the default `boardSize = 10`. The shed itself is not in `tiles`, so do not search for it there
- Plants: `"PLANT" <crop>`, `"WATER"`, `"HARVEST"`, `"FERTILIZE"`
- Animals: `"BUILD_COOP"`, `"BUILD_PASTURE"`, `"FEED"`, `"COLLECT_FERTILIZER"`, `"CARE"`
- Terrain: `"DIG"` (removes a plant, weed, or *empty* coop/pasture from the current tile; a structure with an animal on it cannot be dug — the animal must escape or be left in place)
