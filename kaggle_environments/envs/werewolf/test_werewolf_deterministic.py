...
def deterministic_config_options():
    options = {
        "discussion_protocol": {"name": "RoundRobinDiscussion",
...
def test_game_result(deterministic_agents_config, deterministic_config_options):
    ...
    env = make("werewolf", debug=True,
               configuration={"agents": deterministic_agents_config, **deterministic_config_options})
...
def test_parallel_discussion_simultaneous_majority_vote(deterministic_agents_config, deterministic_config_options):
    ...
    config.update({
        "discussion_protocol": {"name": "ParallelDiscussion",
                                "params": {"ticks": 2}},
        "day_voting_protocol": {"name": "SimultaneousMajority",
                                "params": {"tie_break": TieBreak.NO_EXILE}},
        "werewolf_night_vote_protocol": {"name": "SimultaneousMajority",
                                         "params": {"tie_break": TieBreak.NO_EXILE}}
    })
