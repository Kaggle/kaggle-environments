...
In the Werewolf benchmark, models were evaluated across 31,472 full games, with match participants sampled with replacement to ensure exposure to diverse team compositions and strategy profiles. Evaluating skill in asymmetric, imperfect-information social deduction games presents unique challenges. Conventional evaluation metrics (e.g., overall win rates) and traditional rating algorithms (e.g., Elo~\citep{elo1978rating} and OpenSkill~\citep{herbrich2006trueskill, minka2018trueskill}) systematically conflate an individual agent's skill with the structural base win rates of assigned roles and team variance. We therefore rely on Game Theoretic Evaluation (GTE)~\citep{liu2025re}, which rigorously isolates each model's performance into role-specific contributions.
...
Mapping these capabilities to cost (Figure~\ref{fig:werewolf_pareto}) reveals a clean Pareto frontier anchored by three models: Grok 4.1 Fast Reasoning (low-cost), Gemini 3 Flash Preview (massive performance leap for minimal cost increase), and Gemini 3 Pro Preview (maximum performance). Models such as Claude Opus 4.5 and Grok 4 are strictly dominated, yielding lower strategic performance while being significantly more expensive per game.
...
% redescribe King-of-the-hill game setup in the context of Werewolf
\textbf{Game-Theoretic Evaluation, The King-of-the-Hill Meta-Game:}
To conduct a robust evaluation of Werewolf agents that goes beyond simple average win rates, we employ a game-theoretic evaluation framework known as the ``king-of-the-hill'' meta-game~\cite{liu2025re}. Traditional leaderboards often aggregate performance across all scenarios equally, which can allow agents to achieve high rankings by excelling at frequent but unchallenging tasks while failing at critical ones. Our approach casts evaluation as a competitive interaction between strategic entities, aiming to identify agents that are not only strong on average but also robust across the most discriminative and challenging game roles.

\textbf{Meta-Game Formulation:}
We formulate the evaluation process as a three-player, zero-sum-like meta-game. The players and their respective action sets are defined as follows:
\begin{itemize}
    \item \textbf{The Role Player ($P_{\text{role}}$):} Represents the evaluation arbiter. Its action is to select a role $a_{\text{role}} \in \mathcal{A}_{\text{role}}$ (e.g., Seer, Werewolf, Villager) to serve as the current test case.
    \item \textbf{The King Agent ($P_{\text{king}}$):} Represents the incumbent top-performing agent. Its action is to select an agent policy $a_{\text{king}} \in \mathcal{A}_{\text{agent}}$ to defend its title.
    \item \textbf{The Rebel Agent ($P_{\text{rebel}}$):} Represents the challenger. Its action is to select an agent policy $a_{\text{rebel}} \in \mathcal{A}_{\text{agent}}$ to attempt to dethrone the King.
\end{itemize}

The core of this evaluation lies in the payoff functions, which are derived from the empirical performance of agents in specific roles. Let $v(a_{\text{agent}}, a_{\text{role}}) \in [0, 1]$ denote the empirical win rate of an agent when playing a specific role. This metric is calculated by aggregating binary game outcomes over all recorded matches:
$$v(a_{\text{agent}}, a_{\text{role}}) = \frac{\text{Total Wins}(a_{\text{agent}}, a_{\text{role}})}{\text{Total Games Played}(a_{\text{agent}}, a_{\text{role}})}$$

The utility functions for the three players are designed to align their strategic incentives with the goals of rigorous evaluation:
\begin{align*}
u_{\text{king}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}}) &= v(a_{\text{king}}, a_{\text{role}}) - v(a_{\text{rebel}}, a_{\text{role}}) \\
u_{\text{rebel}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}}) &= -u_{\text{king}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}}) \\
u_{\text{role}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}}) &= |u_{\text{king}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}})|
\end{align*}

In this setup, the \textbf{King} and \textbf{Rebel} play a zero-sum game relative to each other for any given role. The King strives to maximize its performance margin over the Rebel, while the Rebel strives to minimize this margin (or maximize its own advantage). 

Crucially, the \textbf{Role} player is not a passive environment but a strategic actor incentivized to select roles that maximize the absolute performance disparity between the King and Rebel. This strategic objective ensures that the evaluation focuses purely on \textit{discriminative} roles—those that effectively distinguish between agent capabilities—rather than ``easy'' roles where most agents achieve similar win rates.

\textbf{Equilibrium Resolution via Quantal Response:}
To derive a final ranking from this meta-game, we solve for its equilibrium. A standard Nash Equilibrium, which assumes perfect rationality, might focus exclusively on a single, most-discriminative role, rendering the evaluation overly sensitive to statistical noise in that specific role's match outcomes.

Instead, we adopt the \textbf{Quantal Response Equilibrium (QRE)}, specifically a formulation utilizing softmax-based action selection. QRE models bounded rationality, where players choose better strategies with higher probability, but not exclusively. In our context, this means the Role player will assign high probability mass to highly discriminative roles, while still placing some non-zero weight on other relevant roles. 

The resulting equilibrium strategy for the King player provides a robust measure of agent quality. An agent with a high probability mass in the King's equilibrium strategy has proven its ability to maintain a performance advantage even when the Role player actively selects the most challenging comparison points and the Rebel player selects the strongest possible challenger.

\textbf{Variance Estimation and Confidence Intervals}
The empirical win rates, $\hat{v}(a_{\text{agent}}, a_{\text{role}})$, are stochastic estimates derived from a finite number of matches. Acknowledging the inherent uncertainty in these estimates is crucial for a robust evaluation. We quantify this uncertainty using the standard error of the mean for binomial proportion, given by $\sigma_{a,r} = \sqrt{\frac{\hat{v}(1-\hat{v})}{N_{a,r}}}$, where $N_{a,r}$ is the number of games played by agent $a$ in role $r$.

This variance explicitly influences our evaluation in two distinct ways:

\textbf{Regularization via Uncertainty:}
In the Quantal Response Equilibrium (QRE) formulation, higher variance in an agent's win rate acts as a natural regularizer. When solving for equilibrium, the utilities are effectively penalized by their constitutive uncertainty. Consequently, the Role player's equilibrium strategy tends to de-emphasize roles where agent performance is highly noisy, effectively ``softening'' the meta-game to focus on reliable, discriminative signals rather than statistical outliers.

\textbf{Bootstrap Confidence Intervals:}
Because the final King-of-the-Hill ratings are complex, non-linear functions of all underlying win rates, analytical derivation of their confidence intervals is intractable. We instead employ a non-parametric bootstrap approach. We generate $B$ resampled datasets by drawing match outcomes with replacement from the original game logs. For each resampled dataset $b \in \{1, \dots, B\}$, we re-compute the empirical win rate matrix and solve for the QRE ratings. The $95\%$ confidence intervals for each agent's rating are then derived from the $2.5^{\text{th}}$ and $97.5^{\text{th}}$ percentiles of these $B$ bootstrap solutions, providing a rigorous measure of statistical significance for our leaderboard rankings.

\subsection{Werewolf Heuristic Metrics}
\label{app:werewolf_heuristics}

To provide a comprehensive behavioral analysis, we additionally tracked several heuristic metrics, including Key Survival Rate (KSR), Identification Precision (IRP), Voting Success Score (VSS)~\cite{tang2025dsgbench}, Margin of Win, and Speed of Win. While these metrics offer interesting observational perspectives on agent tendencies, our analysis reveals that they are insufficiently informative---and occasionally misleading---when utilized as primary proxies for overall model performance.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Heuristic metrics across evaluated models, demonstrating the limited discriminative variance of heuristics versus ranking methods. The means and 95\% confidence intervals were derived from 1,000 bootstrap iterations across the 31,472 match-ups.}
