...
One of the major challenges facing modern artificial intelligence research is evaluating the performance of large-scale language models (LLMs). Standardized static benchmarks such as MMLU~\citep{hendryckstest2021}, GSM8K~\citep{cobbe2021gsm8k}, and HellaSwag~\citep{zellers2019hellaswag}  have long been important tools for measuring progress in knowledge retrieval, mathematical reasoning, and commonsense reasoning.
However, as the performance of the frontier models on these fixed test sets approaches saturation, their ability to distinguish between different systems decreases~\citep{white2024livebench}. Furthermore, the static nature of these benchmarks makes them susceptible to data contamination that test data may leak into the training corpus, leading to overestimation of performance~\citep{white2024livebench, kapoor2024agents}. 
...
Werewolf is a multiplayer, general-sum social-deduction game with acute information asymmetry. Our implementation of Werewolf features eight players. Each participant is secretly assigned one of the following roles: two Werewolves, one Seer, one Doctor, and four Villagers—forming two opposing teams with fundamentally different strategic profiles. The informed minority (Werewolves) must deceive and divide the majority through natural language persuasion, while the uninformed majority, aided by the Seer's private investigations and the Doctor's protective measures, must argue to identify threats and eliminate them through democratic voting. While Werewolf has countless variations in the literature, Game Arena deliberately utilizes this classic rule set. Increasing rule complexity often has the opposing effect of artificially constraining emergent gameplay, analogous to how the elegant rules of Go yield deeper strategic spaces than other boardgames with heavier mechanics. This setup ensures that complexity naturally emerges from the players' mixed strategies and social dynamics rather than from intricate rule additions. By navigating this ambiguity and varying levels of trust, agents must exercise critical ``soft skills'' such as negotiation, coalition building, and the capacity to detect or engage in strategic deception. Consequently, this dynamic environment serves as a uniquely challenging benchmark for evaluating natural language manipulation and agent safety, fundamentally complementing the silent, two-player settings of Chess and Poker.

The game is led by a moderator who navigates between night phases (private role-related actions) and day phases (public discussion followed by elimination by majority vote). Internally, the environment maintains an authoritative, append-only log of \texttt{Event} records. Each state transition and player interaction (e.g., votes, chat messages, role-specific abilities) is encapsulated within a structured event object that strictly defines its visibility permissions. Information asymmetry is subsequently enforced by a centralized event bus, which filters and dispatches these records to individual agents based on their specific access privileges (e.g., distinguishing between public discussion and private faction communications). A modular protocol system allows the discussion format (e.g., circular vs. parallel) and voting rules (e.g., sequential vs. simultaneous) to be swapped as interchangeable plugins, enabling controlled exploration of the impact of social interaction mechanisms on game balance. We conducted a comprehensive balance analysis (Appendix ~\ref{app:werewolf_balance}) to ensure that our selected configuration of game rules is well-balanced, allowing interesting strategic dynamics to naturally emerge.
...
\begin{figure} 
[p] \centering \includegraphics[ width=\linewidth, height=0.9\textheight, keepaspectratio ]{final_plots/werewolf/gte_contributions.png}
\caption{Role-specific contributions to player skill in Game Theoretic Evaluation. The overall player skill (mean Net Rating) is broken down into the theoretical contributions of individual roles, represented by the widths of the stacked colored bars. Models are ordered by mean Net Rating from bottom (lowest) to top (highest).}
\label{fig:werewolf_gte_contributions}
\end{figure}

\begin{figure} 
[p] \centering \includegraphics[ width=\linewidth, height=0.9\textheight, keepaspectratio ]{final_plots/werewolf/gte_ratings.png}
\caption{Bootstrapped distributions of the Net Rating for each model, derived from 1,000 bootstrap iterations across 31,472 match-ups. Dashed lines within each violin denote the quartiles and median. The vertical dashed line at 0 indicates the break-even rating, and models are ordered by mean Net Rating from bottom (lowest) to top (highest).}
\label{fig:werewolf_gte_ratings}
\end{figure}
...
\caption{Cost-performance Pareto frontier based on Game Theoretic Evaluation. The dashed line connects the frontier models (Grok 4.1 Fast Reasoning, Gemini 3 Flash Preview, and Gemini 3 Pro Preview), highlighting optimal trade-offs between average inference cost per game and overall GTE rating.}
\label{fig:werewolf_pareto}
\end{figure}

In the Werewolf benchmark, models were evaluated across 31,472 full games, with match participants sampled with replacement to ensure exposure to diverse team compositions and strategy profiles. Evaluating skill in a multiplayer, general-sum social deduction game with asymmetric, imperfect-information presents unique challenges. While traditional rating algorithms like Elo~\citep{elo1978rating} and OpenSkill~\citep{herbrich2006trueskill, minka2018trueskill} are highly effective in the symmetric environments they were designed for, applying them directly to multi-agent settings with asymmetric role abilities and team sizes routinely conflates an individual agent's core skill with the structural base win rates of their randomly assigned roles. Because this environment requires specific modeling to account for such structural variance, we utilize Game Theoretic Evaluation (GTE)~\citep{liu2025re}, which rigorously isolates each model's performance into role-specific contributions.

To contextualize GTE's effectiveness, we benchmarked it against group Elo and OpenSkill (Figure~\ref{fig:werewolf_ratings}). While the relative macro-rankings remain highly consistent across methods, GTE demonstrates superior discriminative resolution. Group Elo and OpenSkill struggle to differentiate mid-tier performance---frequently producing overlapping confidence intervals---whereas the GTE framework successfully isolates role-specific strategic value, resulting in a cleaner, statistically significant separation between models.

Performance under the GTE framework is sharply stratified (Figure~\ref{fig:werewolf_gte}). At the top tier, Gemini 3 Pro Preview and Gemini 3 Flash Preview achieve the highest net ratings, demonstrating balanced, positive win-rate contributions across both the informed minority (Werewolf) and uninformed majority (Villager, Seer, Doctor) roles. In the mid-tier, distinct strategic weaknesses emerge; for example, Claude Sonnet 4.5 and Grok 4.1 Fast Reasoning exhibit substantial negative contributions when assigned the Seer role, suggesting difficulty in communicating private information without drawing suspicion. At the bottom tier, GPT-5 mini displays severe negative contributions across all roles, indicating systemic weaknesses in both collaborative deduction and deceptive persuasion.

Mapping these capabilities to cost (Figure~\ref{fig:werewolf_pareto}) reveals a clean Pareto frontier anchored by three models: Grok 4.1 Fast Reasoning (low-cost), Gemini 3 Flash Preview (substantial performance leap for minimal cost increase), and Gemini 3 Pro Preview (maximum performance). This frontier highlights the optimal trade-offs between average inference cost per game and overall strategic capability within the evaluated pool.
...
\caption{Ratings of group Elo, OpenSkill, and Game Theoretic Eval (GTE). The means and 95\% confidence intervals were derived from 1,000 bootstrap iterations across the 31,472 match-ups. Even though the relative rankings and trends stay consistent across the three methods, Game Theoretic Evaluation exhibits tighter confidence intervals and greater discriminative power, whereas group Elo and OpenSkill struggle to distinctly separate the top-performing models.}
\label{fig:werewolf_ratings}
\end{figure}

Finally, we tracked several heuristic metrics, including Key Survival Rate (KSR), Identification Precision (IRP), Margin of Win, and Voting Success Score (VSS) (see Figure~\ref{fig:heuristic_metrics}, and ~\ref{app:werewolf_heuristics}). However, we found them to be insufficiently informative, and occasionally misleading, when utilized as primary proxies for overall model performance. For instance, a strict survival metric (KSR) fundamentally penalizes sophisticated plays, as strategically sacrificing a key role is sometimes optimal for securing a team victory. Similarly, IRP and VSS lack discriminative variance because capable models quickly learn to converge on consensus targets during the discussion phase to avoid broadcasting their identity; consequently, final votes largely reflect group conformity rather than private deductive accuracy. These limitations reinforce our reliance on the Game Theoretic Evaluation framework as the definitive, unbiased measure of agent skill.
...
\subsection{Werewolf Game Rules and Balanced Configuration}
\label{app:werewolf_rules}

The Werewolf evaluation environment in Game Arena utilizes a specific, deliberately balanced variant of the classic social deduction game, configured to foster complex strategic interactions without relying on overly intricate mechanics. The standard match comprises eight players randomly assigned to two opposing factions: the informed minority (the Werewolf team) and the uninformed majority (the Village team).

\subsubsection{Roles and Objectives}
The eight-player pool is partitioned into two Werewolves, one Seer, one Doctor, and four Villagers. The Werewolves' objective is to eliminate members of the Village until their numbers are equal, thereby securing a majority. Conversely, the Village team aims to identify and sequentially exile all Werewolves. With the exception of the Werewolves, who possess shared knowledge of their faction members, all other players start the game with no information regarding the identities of their peers.

\subsubsection{Game Phases and Action Protocols}
The game alternates between a private ``Night'' phase and a public ``Day'' phase. 
During the Night phase, roles with specialized abilities act in a strict sequence:
\begin{enumerate}
    \item \textbf{Doctor:} The Doctor silently selects one player to protect from elimination. To prevent degenerate survival strategies, the Doctor is strictly prohibited from targeting themselves (\textit{no self-save}) and may not protect the same player in consecutive nights.
    \item \textbf{Seer:} The Seer selects one player to investigate, learning the exact secret role of that target.
    \item \textbf{Werewolves:} The Werewolves coordinate to select a target for elimination. In the balanced configuration, this is executed via sequential voting, where each Werewolf votes with full visibility of their partner's prior vote. The first voter rotates each night. If the vote results in a tie, no elimination occurs (\textit{no elected} tie-breaker).
\end{enumerate}

Following the Night, the Day phase commences with the moderator publicly announcing any eliminations that occurred. The eliminated player is removed from the game, and their secret role is fully revealed to all surviving participants. The surviving players then engage in public deliberation. To ensure equitable participation and structured debate, the discussion follows a Round-Robin protocol constrained to a maximum of two full cycles. The order in which players speak rotates systematically each day.

After the discussion concludes, the Day phase culminates in the Exile Vote. Similar to the Night phase, the village conducts a sequential vote, with the voting order rotating each day, to select a player for execution. If a plurality target is successfully chosen, that player is exiled, and their role is publicly disclosed. In the event of a tied vote, no player is exiled for that day (\textit{no elected} tie-breaker). The game then loops back to the Night phase, continuing this cycle until either the Village successfully purges all Werewolves or the Werewolves achieve numeric parity with the Village.
...
\begin{figure}[H]
    ...
    \caption{Tournament graph illustrating the pairwise win margins between the eight evaluated models. A directed edge from model $A$ to model $B$ indicates that $A$ achieved more pairwise victories against $B$, with the edge label quantifying the net vote differential $\delta(A,B)$. Notably, the resulting graph is perfectly transitive and contains no cycles, establishing a clean, unambiguous dominance hierarchy among the models.}
    \label{fig:werewolf_tournament_graph}
\end{figure}

To analyze the relative capabilities of the agents, we ground our methodology in the Voting-as-Evaluation (VasE) framework~\citep{lanctot2023evaluating}. The VasE framework reinterprets multi-agent evaluation as a social choice problem, mapping distinct game scenarios to ``voters'' and the models under evaluation to ``candidates''. Under this paradigm, we aggregate binary pairwise comparisons across all match-ups and roles: whenever model $a$ achieves a higher match score than model $b$ in a specific assignment, that configuration acts as a voter casting a ballot for candidate $a$. These head-to-head outcomes form the foundational input for our Elo rating calculations and alternative ranked voting metrics.

To visualize the pairwise dominance relationships between the candidate models, we construct a tournament graph (Figure~\ref{fig:werewolf_tournament_graph}) featuring directed edges from $a$ to $b$ wherever the win differential $\delta(a,b) = N(a,b) - N(b,a) > 0$. Here, $N(a,b)$ represents the total number of pairwise victories model $a$ holds against $b$, and the edge weights explicitly display this net differential. This approach mirrors the initial tallying step of the Ranked Pairs voting method~\citep{tideman1987independence}.

Crucially, the resulting tournament graph is completely acyclic. This strict transitivity establishes a clean dominance hierarchy among the models, providing an optimal condition for converging Elo rating algorithms and eliminating the need for complex cycle-resolution heuristics. Using these pairwise margins, we also evaluate the models using Copeland's method~\citep{copeland1951reasonable}, computing a Copeland score, $CS(a)$, defined mathematically as $1 \times |\{b : \delta(a,b) > 0\}| + 0.5 \times |\{b : \delta(a,b) = 0\}|$. This score simply tallies the number of pairwise matchups a candidate wins (scoring 1) or ties (scoring 0.5). Additionally, we establish the explicit Ranked Pairs outcome by progressively removing sources from the graph to establish a ranking sequence.

As detailed in Table~\ref{tab:alternative_rankings}, the rankings produced by these alternative voting-based methods and the dataset-derived Elo ratings remain perfectly consistent with both the official Game Theoretic Evaluation rankings and the leaderboard reported on the Kaggle Game Arena site.

\begin{table}[H]
    \centering
    \begin{tabular}{l c c c}
        \hline
        \textbf{Model} & \textbf{Elo Rating} & \textbf{Copeland Score} & \textbf{Ranked Pairs Rank} \\
        \hline
        Gemini 3 Pro Preview & 151 & 7.0 & 1 \\
        Gemini 3 Flash Preview & 124 & 6.0 & 2 \\
        GPT-5.2 & 122 & 5.0 & 3 \\
        Claude Opus 4.5 & 110 & 4.0 & 4 \\
        Claude Sonnet 4.5 & 96 & 3.0 & 5 \\
        Grok 4.1 Fast Reasoning & 50 & 2.0 & 6 \\
        Grok 4 & 39 & 1.0 & 7 \\
        GPT-5 mini & 0 & 0.0 & 8 \\
        \hline
    \end{tabular}
    \vspace{2mm}
    \caption{Comparison of alternative ranking metrics derived from the pairwise win margin dataset. The models are identically ordered across Elo, Copeland's method, and Ranked Pairs.}
    \label{tab:alternative_rankings}
\end{table}

\subsubsection{Group Elo Implementation for Multiplayer Matches}
To compute the Elo ratings referenced in our transitivity analysis, we adapt the standard pairwise Elo rating system to accommodate the asymmetric, multi-agent team structure of Werewolf. In each match, the agents are partitioned into two opposing factions: the Villagers ($V$) and the Werewolves ($W$). 

First, we establish a collective team rating by computing the mean Elo rating of the constituent agents for each faction:
\begin{align*}
R_V &= \frac{1}{|V|} \sum_{a \in V} R_a \\
R_W &= \frac{1}{|W|} \sum_{a \in W} R_a
\end{align*}
Using these aggregated team ratings, the expected win probability for the Villagers, $E_V$, is strictly calculated via the standard logistic curve:
$$E_V = \frac{1}{1 + 10^{(R_W - R_V)/400}}$$

Following the conclusion of the match, the actual outcome for the Villager team is denoted as $S_V \in \{0, 1\}$ (where $1$ indicates a Villager victory and $0$ indicates a Werewolf victory). The total rating points to be exchanged between the two factions is then determined using a base $K$-factor of 32:
$$\Delta = 32 \times (S_V - E_V)$$

To ensure the system remains zero-sum while fairly crediting individual participation, this total rating delta is distributed equally among the members of each respective team. Because the factions are asymmetric in size (typically $6$ Villagers and $2$ Werewolves), the individual rating updates for each agent $a$ are explicitly scaled by the inverse of their faction's precise size:
\begin{align*}
R_a &\leftarrow R_a + \frac{\Delta}{|V|} \quad \text{for } a \in V \\
R_a &\leftarrow R_a - \frac{\Delta}{|W|} \quad \text{for } a \in W
\end{align*}
This group Elo formulation tracks cumulative strategic performance while robustly controlling for variable team sizes and multi-role configurations.
...
\textbf{Game-Theoretic Evaluation: The King-of-the-Hill Meta-Game}
To conduct a robust evaluation of Werewolf agents that extends beyond simple average win rates, we employ a game-theoretic framework formulated as a ``king-of-the-hill'' meta-game~\citep{liu2025re}. While traditional leaderboards aggregate performance across all scenarios equally---allowing agents to achieve high rankings by excelling at frequent but unchallenging tasks---our approach casts evaluation as a competitive interaction. This isolates agents that are not only strong on average, but distinctly robust across the most challenging, discriminative game roles.

\textbf{Meta-Game Formulation:}
We map the evaluation process into a three-player, zero-sum-esque meta-game. The players and their respective action sets are defined as follows:
\begin{itemize}
    \item \textbf{The Role Player ($P_{\text{role}}$):} Acts as the evaluation arbiter, selecting a role $a_{\text{role}} \in \mathcal{A}_{\text{role}}$ (e.g., Seer, Werewolf, Villager) to serve as the test case.
    \item \textbf{The King Agent ($P_{\text{king}}$):} The incumbent top-performing agent, selecting a policy $a_{\text{king}} \in \mathcal{A}_{\text{agent}}$ to defend its superiority.
    \item \textbf{The Rebel Agent ($P_{\text{rebel}}$):} The challenger, selecting a policy $a_{\text{rebel}} \in \mathcal{A}_{\text{agent}}$ in an attempt to dethrone the King.
\end{itemize}

The core of this evaluation relies on payoff functions derived from empirical win rates. Let $\hat{v}(a_{\text{agent}}, a_{\text{role}}) \in [0, 1]$ denote the win probability of an agent occupying a specific role, calculated by aggregating binary outcomes over all recorded match-ups:
$$ \hat{v}(a_{\text{agent}}, a_{\text{role}}) = \frac{\text{Total Wins}(a_{\text{agent}}, a_{\text{role}})}{\text{Total Games Played}(a_{\text{agent}}, a_{\text{role}})} $$

The utility functions cleanly align the players' strategic incentives with rigorous evaluation:
\begin{align*}
u_{\text{king}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}}) &= \hat{v}(a_{\text{king}}, a_{\text{role}}) - \hat{v}(a_{\text{rebel}}, a_{\text{role}}) \\
u_{\text{rebel}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}}) &= -u_{\text{king}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}}) \\
u_{\text{role}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}}) &= \left| u_{\text{king}}(a_{\text{role}}, a_{\text{king}}, a_{\text{rebel}}) \right|
\end{align*}

In this dynamic, the \textbf{King} and \textbf{Rebel} play a zero-sum game conditioned on the role: the King strives to maximize its performance margin over the Rebel, while the Rebel strives to minimize it. Concurrently, the \textbf{Role} player acts not as a passive environment, but as an adversary incentivized to select roles that maximize the absolute performance disparity between King and Rebel. This ensures the focus remains on \textit{discriminative} roles rather than uninformative scenarios where all agents exhibit similar baseline win rates.

\textbf{Equilibrium Resolution via Quantal Response:}
To resolve a definitive ranking, we calculate the meta-game's equilibrium. Relying on a standard Nash Equilibrium (assuming perfect rationality) could force the evaluation to overfit exclusively on the single most-discriminative role, magnifying statistical noise. 

Instead, we adopt a \textbf{Quantal Response Equilibrium (QRE)} utilizing a softmax-based choice model. QRE models bounded rationality, ensuring players choose superior strategies with higher probability without committing exclusively. In practice, the Role player assigns high probability mass to highly discriminative roles while retaining non-zero weight on others. The resulting King-side equilibrium provides a stable, robust measure of holistic agent quality: an agent securing a high probability mass has proven its capability to maintain an advantage, even when actively challenged by the strongest opposing Rebel in the most difficult Roles.

\textbf{Variance Estimation and Confidence Intervals:}
We acknowledge that empirical win rates are stochastic estimates bounded by finite match histories. To quantify this uncertainty, we compute the standard error of the mean for binomial proportions: $\sigma_{a,r} = \sqrt{\smash[b]{\frac{\hat{v}(1-\hat{v})}{N_{a,r}}}}$, where $N_{a,r}$ is the sample size for agent $a$ in role $r$.

This variance explicitly regularizes the QRE formulation: utilities are effectively penalized by their constitutive uncertainty. Consequently, the Role player's strategy inherently de-emphasizes roles prone to highly noisy outcomes, softening the meta-game to focus purely on reliable, clean signals. 

Because final GTE ratings are complex, non-linear derivatives of the underlying win rate matrix, analytically computing confidence intervals is intractable. We instead employ a non-parametric bootstrap approach: generating $B=1,000$ resampled datasets (drawing match outcomes with replacement), re-computing the empirical win rates, and solving for QRE ratings per sample. The 95\% confidence intervals for each agent's final rating are extracted from the $2.5^{\text{th}}$ and $97.5^{\text{th}}$ percentiles across the $B$ iterations, assuring statistically rigorous leaderboard margins.

\subsection{Werewolf Heuristic Metrics}
\label{app:werewolf_heuristics}

To provide a comprehensive behavioral analysis, we tracked additional heuristic metrics, including Key Survival Rate (KSR), Identification Precision (IRP), Voting Success Score (VSS)~\citep{tang2025dsgbench}, Margin of Win, and Speed of Win. While these metrics offer interesting perspectives on agent tendencies, our analysis reveals that they are insufficiently informative---and occasionally misleading---when utilized as primary proxies for overall model performance.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Breakdown of heuristic metrics across evaluated models, demonstrating the limited discriminative variance of algorithmic heuristics compared to holistic ranking methods. Note that for most heuristics (except KSR), scores are saturated or uniform across varying model tiers, revealing their insufficiency in resolving strategic depth. The means and 95\% confidence intervals were derived from 1,000 bootstrap iterations across the 31,472 match-ups.}
    \label{fig:heuristic_metrics}
\end{figure}

Because these heuristics are heavily skewed by game structure and consensus incentives, they fail to isolate true capability. This finding reinforces the critical utility of the Game Theoretic Evaluation (GTE) framework as our definitive, unbiased measure of agent skill.
