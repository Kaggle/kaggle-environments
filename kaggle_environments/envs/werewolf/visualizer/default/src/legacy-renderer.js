...
import {
  ...
  createPlayerCapsule,
  replacePlayerIdsWithCapsules,
  replacePlayerIdsWithBold,
  getThreatColor,
  updatePlayerList,
  updateEventLog,
  renderPlayerList } from './functions/helpers.js';

import {
  createSkyControlsPanel,
  toggleSkyControls,
  updateSkyParameter,
  updateSunPosition,
  updateExposure,
  ...
  toggleSkyTransition,
  updateSkyInfo
} from './functions/skyControlFunctions.js'

export function renderer(context, parent) {
  const { replay, step, height = 1000, width = 1500 } = context;
  const environment = replay;

  // This is a temporary state check for debugging initialization.
  if (window.werewolfThreeJs) {
    console.log(
      '[DIAGNOSTIC] renderer() called for step:',
      step,
      '| players3DInitialized:',
      window.werewolfThreeJs.players3DInitialized
    );
  } else {
    console.log(
      '[DIAGNOSTIC] renderer() called for step:',
      step,
      '| players3DInitialized: false (threeState not created yet)'
    );
  }
  ...
  if (!window.werewolfGamePlayer) {
    ...
    // Update the steps on the context for the current render pass.
    context.replay.steps = newSteps;
    environment.steps = newSteps;

    // Notify the core player to update its state (e.g., the slider).
  ...
  if (context.unstable_replayerControls && !window.werewolfControlsPatched) {
    ...
    playerInstance.play = (continuing) => {
      ...
      if (audioState.isAudioEnabled) {
        ...
        originalPause(); // Pause the timer-based playback
        playerInstance.setPlayingState(true); // Set state without starting a timer
    ...
    playerInstance.pause = () => {
      ...
      originalPause(); // Stops the core timer

      // This is now safe to call because the adapter implementation was changed.
    ...
    const loadAndSetup = async () => {
      try {
        ...
        const { FBXLoader } = await import(
          'https://cdn.jsdelivr.net/npm/three@0.118.1/examples/jsm/loaders/FBXLoader.js'
        );
        const { SkeletonUtils } = await import(
          'https://cdn.jsdelivr.net/npm/three@0.118.1/examples/jsm/utils/SkeletonUtils.js'
        );
        const { EXRLoader } = await import(
          'https://cdn.jsdelivr.net/npm/three@0.118.1/examples/jsm/loaders/EXRLoader.js'
        );
        const { CSS2DRenderer, CSS2DObject } = await import(
          'https://cdn.jsdelivr.net/npm/three@0.118.1/examples/jsm/renderers/CSS2DRenderer.js'
        );
        const { EffectComposer } = await import(
          'https://cdn.jsdelivr.net/npm/three@0.118.1/examples/jsm/postprocessing/EffectComposer.js'
        );
        const { RenderPass } = await import(
          'https://cdn.jsdelivr.net/npm/three@0.118.1/examples/jsm/postprocessing/RenderPass.js'
        );
        const { UnrealBloomPass } = await import(
          'https://cdn.jsdelivr.net/npm/three@0.118.1/examples/jsm/postprocessing/UnrealBloomPass.js'
        );
        const { ShaderPass } = await import(
          'https://cdn.jsdelivr.net/npm/three@0.118.1/examples/jsm/postprocessing/ShaderPass.js'
        );
        const { FilmPass } = await import(
          'https://cdn.jsdelivr.net/npm/three@0.118.1/examples/jsm/postprocessing/FilmPass.js'
        );
        ...
        class BasicWorldDemo {
          ...
          _updateSkySystem(phase) {
            ...
            console.debug(
              '[SKY DEBUG] Sky shader sunPosition uniform:',
            ...
            if (this._moonMesh) {
              ...
              if (this._moonGlow && this._moonGlow.material) {
                this._moonGlow.material.opacity = moonElevation > 0 ? 0.15 * Math.max(0, Math.sin(moonElevation)) : 0;
              }
            }
            ...
            if (phase <= 0.5) {
              ...
              if (dayProgress < 0.1 || dayProgress > 0.9) {
                ...
                skyUniforms['mieCoefficient'].value =
                  mieCoeff_dusk + (mieCoeff_noon - mieCoeff_dusk) * transitionFactor;
          ...
          _createCampfire(THREE) {
            ...
            if (this._VolumetricFire) {
              this._VolumetricFire.texturePath = '../static/volumetric_fire/textures/';
            }
            ...
            if (this._VolumetricFire) {
              this._fire = new this._VolumetricFire(fireWidth, fireHeight, fireDepth, sliceSpacing, this._camera);
            ...
            for (let i = 0; i < logCount; i++) {
              ...
              const logGeometry = new THREE.CylinderGeometry(logRadius, logRadius * 0.9, logLength, 8);
            ...
            for (let i = 0; i < kindlingCount; i++) {
              ...
              const kindlingGeometry = new THREE.CylinderGeometry(0.03, 0.025, 0.4 + Math.random() * 0.3, 6);
          ...
          _loadTownModel(THREE, FBXLoader) {
            ...
            const townModelPath = '../static/werewolf/town/scene_v1.fbx'; // <-- REPLACE WITH YOUR FILE PATH
            ...
            this._fbxLoader.load(
              ...
              (progress) => {
                ...
                console.debug(
                  '[Town Loader] Loading progress: ' + ((progress.loaded / progress.total) * 100).toFixed(2) + '%'
                );
              },
              (error) => {
          ...
          _load_ground(THREE, EXRLoader) {
            ...
            const roughnessTexture = exrLoader.load('../static/werewolf/ground/rocky_terrain_02_rough_1k.exr');
            const normalTexture = exrLoader.load('../static/werewolf/ground/rocky_terrain_02_nor_gl_1k.exr');
            ...
            const modelPromise = new Promise((resolve, reject) => {
              ...
              this._fbxLoader.load(
                ...
                (progress) => {
                  ...
                  console.debug(
                    `Loading merged model for ${normalizedRole}: ${((progress.loaded / progress.total) * 100).toFixed(2)}%`
                  );
                },
                (error) => {
          ...
          loadCharacterAnimations(role) {
            ...
            console.warn(
              `Animations for ${normalizedRole} not yet loaded. They will be extracted from the merged FBX.`
            );
          ...
          _loadAnimationWithFallbacks(role, animationName) {
            console.debug(
              `_loadAnimationWithFallbacks called for ${role}/${animationName} - this should not happen with merged FBX files`
            );
          ...
          playAnimation(playerName, animationName, options = {}) {
            ...
            if (deathMap && deathMap.has(playerName)) {
              ...
              if (!['Victory', 'Defeated'].includes(animationName)) {
                console.debug(
                  `[BLOCKED] Animation '${animationName}' blocked for ${playerName} - death animation already completed`
                );
                return null;
              }
            }
          ...
          updateVoteVisuals(votes, clearAll = false) {
            ...
            votes.forEach((voteData, voterName) => {
              ...
              if (type === 'night_vote')
                color = 0xff0000; // Red
              else if (type === 'doctor_heal_action')
          ...
          _updateSceneForPhase(phaseValue) {
            ...
            if (this._bloomPass) {
              ...
              this._bloomPass.threshold = 0.0 + phaseValue * 0.1; // User base: 0.00, slight increase at night (0.00 to 0.10)
            }
          }
  ...
  function setupScene(BasicWorldDemo, VolumetricFire) {
    ...
    if (
      threeState.demo &&
      threeState.demo._playerObjects &&
      !threeState.players3DInitialized &&
      playerNamesFor3D.length > 0
    ) {
      console.log('[DIAGNOSTIC] Starting player initialization block from setupScene.');
      ...
      initializePlayers3D(gameState, playerNamesFor3D, playerThumbnailsFor3D, threeState)
        .then(() => {
          console.log('[DIAGNOSTIC] ASYNC INIT COMPLETE. Calling updateSceneFromGameState from setupScene.');
  ...
  function updateSceneFromGameState(gameState, playerMap, actingPlayerName) {
    ...
    if (isVotingSession) {
      ...
      for (const event of relevantEvents) {
        if (
          event.type === 'vote' ||
          event.type === 'night_vote' ||
          event.type === 'doctor_heal_action' ||
          event.type === 'seer_inspection'
        ) {
    ...
    if (threeState.demo._spotLight) {
      ...
      const nightActor =
        gameState.game_state_phase === 'NIGHT' &&
        lastEvent &&
        lastEvent.actor_id &&
        ['WerewolfNightVoteDataEntry', 'DoctorHealActionDataEntry', 'SeerInspectActionDataEntry'].includes(
          lastEvent.dataType
        )
          ? lastEvent.actor_id
          : null;
    ...
    if (lastEvent) {
      ...
      if (lastEvent.event_name === 'moderator_announcement') {
        ...
        if (
          (lastEvent.type === 'vote' || lastEvent.type === 'night_vote') &&
          threeState.demo.triggerPointingAnimation
        ) {
      ...
      if (historyEvent.event_name === 'vote_action') {
        ...
        if (match) {
          ...
          gameState.eventLog.push({
            ...
            actor_id: actor_id,
            reasoning: 'Timed out',
            timestamp: historyEvent.created_at,
          });
        }
    ...
    switch (historyEvent.dataType) {
      ...
      case 'DoctorSaveDataEntry':
        gameState.eventLog.push({
          ...
          day: historyEvent.day,
          phase: historyEvent.phase,
          saved_player: data.saved_player_id,
          allEventsIndex: i,
          timestamp,
        });
        break;
  ...
  // Update 3D scene based on game state, only if players are already initialized.
  console.log('[DIAGNOSTIC] Checking whether to run main scene update. Flag:', threeState.players3DInitialized);
  if (threeState.players3DInitialized) {
    updateSceneFromGameState(gameState, playerMap, nameToHighlight);
  }
}
