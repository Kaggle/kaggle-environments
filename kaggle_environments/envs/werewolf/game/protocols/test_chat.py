...
from kaggle_environments.envs.werewolf.game.protocols.chat import RoundRobinDiscussion, FirstToSpeak
import pytest
...
def test_rotate():
    ...
    expected = ["p0", "p1", "p2", "p3", "p0", "p1", "p2", "p3"]
    assert list(protocol._queue) == expected
    assert protocol._current_cursor == 1
    ...
    assert protocol._current_cursor == 2
...
def test_default_is_rotate():
    ...
    assert protocol.first_to_speak == FirstToSpeak.ROTATE
...
def test_round_robin_rotate_skip_dead():
    ...
    assert protocol._current_cursor == 3
    ...
    assert protocol._current_cursor == 0
