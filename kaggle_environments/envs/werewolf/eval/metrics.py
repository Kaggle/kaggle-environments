...
def _get_git_hash() -> str:
    ...
    try:
        ...
        metrics_path = os.path.join(eval_dir, "metrics.py")
        loaders_path = os.path.join(eval_dir, "loaders.py")
        cmd = ['git', 'log', '-n', '1', '--pretty=format:%H', '--', metrics_path, loaders_path]
        _cached_git_hash = subprocess.check_output(cmd).strip().decode('utf-8')
    except Exception:
...
class GameSetEvaluator:
    ...
    def _bootstrap_elo(self, num_samples=100):
        ...
        # Check cache
        cache_key = f"elo_{self.git_hash}_{self.input_hash}_{self.seed}_{num_samples}_v5.pkl"
        cache_path = self.cache_dir / cache_key
    ...
    def _bootstrap_openskill(self, num_samples=100):
        ...
        # Check cache
        cache_key = f"openskill_{self.git_hash}_{self.input_hash}_{self.seed}_{num_samples}_v5.pkl"
        cache_path = self.cache_dir / cache_key
    ...
    def _run_gte_evaluation(self, num_samples: int, light_gte_games: List[LightGame] = None):
        ...
        cache_key = f"gte_{self.git_hash}_{self.input_hash}_{self.seed}_{num_samples}_{tasks_hash}_v5.pkl"
        cache_path = self.cache_dir / cache_key
...
if __name__ == '__main__':
    ...
    import kaggle_environments.envs.werewolf.eval.plotting as plts
    ...
    if args.plot_backend == "plotly":
        plts.plot_metrics(evaluator, [str(output_dir / "metrics.html"), str(output_dir / "metrics.png")])
        plts.plot_gte_evaluation(evaluator, [str(output_dir / "gte.html"), str(output_dir / "gte.png")])
        plts.plot_gte_metrics_analysis(evaluator, [str(output_dir / "gte_metrics.html"), str(output_dir / "gte_metrics.png")])
        plts.plot_pareto_frontier(evaluator, [str(output_dir / "pareto.html"), str(output_dir / "pareto.png")])
        plts.plot_tournament_graph(evaluator, [str(output_dir / "tournament_graph.html"), str(output_dir / "tournament_graph.png")])
        plts.plot_pairwise_winrates(evaluator, [str(output_dir / "pairwise_winrates.html"), str(output_dir / "pairwise_winrates.png")])
    elif args.plot_backend == "seaborn":
        plts.plot_metrics_paper(evaluator, [str(output_dir / "metrics.png"), str(output_dir / "metrics.pdf")])
        plts.plot_gte_evaluation_paper(evaluator, [str(output_dir / "gte.png"), str(output_dir / "gte.pdf")])
        plts.plot_gte_metrics_analysis_paper(evaluator, [str(output_dir / "gte_metrics.png"), str(output_dir / "gte_metrics.pdf")])
        plts.plot_pareto_frontier_paper(evaluator, [str(output_dir / "pareto.png"), str(output_dir / "pareto.pdf")])
        plts.plot_tournament_graph_paper(evaluator, [str(output_dir / "tournament_graph.png"), str(output_dir / "tournament_graph.pdf")])
        plts.plot_pairwise_winrates_paper(evaluator, [str(output_dir / "pairwise_winrates.png"), str(output_dir / "pairwise_winrates.pdf")])
